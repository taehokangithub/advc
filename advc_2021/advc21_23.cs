...
class Advc21_23
{
	...
	class Board
	{
		...
		void SolveInternal(SolveStateList stateList)
		{
			Dictionary<string, SolveStateList> visited = new Dictionary<string, SolveStateList>();
			Dictionary<string, SolveStateList> candidates = new Dictionary<string, SolveStateList>();

			SolveStateList? nextList = stateList;

			int cntVisit = 0;
			while(nextList != null)
			{
				SolveState state = nextList.Last;

				if (CheckAnswerAndSave(nextList))
				{
					return;
				}
				...
				if (++cntVisit % 1000 == 0)
				{
					Console.WriteLine($"Visiting [{cntVisit}] moves[{nextList.Answer.Count}] visitied {visited.Count} candidate {candidates.Count}");
				...
				foreach (Beast beast in state.Beasts.Values)
				{
					...
					{
						SolveState newState = new SolveState(nextList.Last);
						newState.MoveBeastLocation(beast.Loc, point);

						string str = newState.BeastPosRepresentation();
						...
						if (!candidates.ContainsKey(str) || candidates[str].Cost > newState.Cost)
						{
							SolveStateList newStateList = new SolveStateList(nextList);
							newStateList.AddNewState(newState);

							candidates[str] = newStateList;
						}			
					}
				}
				...
				foreach (SolveStateList list in candidates.Values.OrderBy(l => l.Cost))
				{
					nextList = list;
					string str = list.Last.BeastPosRepresentation();
					candidates.Remove(str);
					visited[str] = nextList;
					break;
				}
			}

			throw new Exception("No ore candidates - Could not find the answer");
		}
		...
		public void PrintSolveState(SolveState state)
		{
			...
			for (int y = 0; y < m_max.Y; y ++)
			{
				for (int x = 0; x < m_max.X; x ++)
				{
					...
					if (state.HasBeast(p))
					{
						if (state.GetBeast(p).Fixed)
						{
							Console.Write($"[{state.GetBeast(p).Type}]");
						}
						else
						{
							Console.Write($" {state.GetBeast(p).Type} ");
	...
	static void Run()
	{
		...
		SolveMain($"../../data/{className}_2.txt");
	}
