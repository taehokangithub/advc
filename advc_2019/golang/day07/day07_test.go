...
func TestSeqGenerator(t *testing.T) {
	...
	if len(ret) != 120 {
		t.Errorf("Picking 5 elements returns %d cases - expected 120\n", len(ret))
	}

	ret = pickN(3)

	if len(ret) != 6 {
		t.Errorf("Picking 5 elements returns %d cases - expected 6\n", len(ret))
	}
}
...
func TestThruster(t *testing.T) {
	...
	for i, c := range cases {
		got := solve01(c.memory)
		if got != c.ans {
			t.Errorf("case %d got %d expected %d", i, got, c.ans)
		}
	}
}
